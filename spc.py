"""get, analyze, plot storm reports from SPC"""
...
def getTCTOR(rename=True):
    ...
    REdf = pd.read_csv(ifile,
                       ...
                       skiprows=1,
                       header=[0],
                       nrows=1506)  # skipfooter=23)
    # skipfooter=23 could be used, but 'c' engine doesn't handle it. 'c'
    # engine is faster than 'python' engine.
    ...
    cat_type = CategoricalDtype(
        categories=["N", "TD", "TS", "H", "MH"], ordered=True)
...
def spc_lsr_filename(event_type, latestyear=2020):
    """
     Get wind, hail, or tornado local storm reports (LSR). If LSR do not exist locally,
     download from SPC.
     Cache in TMPDIR, which by default is /glade/scratch/$USER/temp/.

     Input
     event_type: "wind", "hail", or "torn"
     latestyear: most recent (4-digit) year in SPC dataset. Part of SPC
     filename. Update when SPC releases new year.
    """
...
def get_storm_reports(
    ...
    for event_type in event_types:
        ...
        rpts_file, nominal_last_time = spc_lsr_filename(
            event_type, latestyear=latestyear)

        # Make sure the requested last time isn't later than the nominal last
        # time.
        if end > nominal_last_time:
            logging.error(
                f"spc.get_storm_reports({event_type}): "
                f"requested end time {end} is later than nominal last time "
                f"in {rpts_file} {nominal_last_time}")
            logging.error(
                f"Do you need to download a new {event_type} database from SPC?")
            sys.exit(1)
        ...
        dtype = {
            ...
            "stn": np.int64,  # State number - number of tornado in state this year
            "mag": np.float64,  # might think "sz" for hail "f" for torn, but all "mag"
            "inj": np.int64,
            "fat": np.int64,
            "loss": np.float64,
            "closs": np.float64,
        ...
        # Unify "date" and "time" to make a naive datetime object
        ...
        """
        "Significant": tornadoes rated EF2 or greater,
        thunderstorm wind gusts of hurricane force (74 mph) or higher, or hail 2
        ...
        # Derive timezone-aware datetime "time" from "date_time" and "tz" columns.
        # Convert to UTC.
        ...
        logging.debug(
            f"spc.get_storm_reports(): found {unknown_tz.sum()} {event_type} "
            "in unknown time zone")
        logging.debug(
            rpts.loc[unknown_tz, ['om', 'date_time', 'tz', 'event_type', 'source']])

        """
        Wrote to SPC Apr 1 2019 about fixing tz!=3 lines.
        Email20190401 PatrickMarsh took over database in 2017 and has no record
        or documentation as to what those timezones are.
        Each year I append new information to the end of the old information,
        so timezones will continue to exist as is until I can learn what those
        time zones are.
        ...
        When timezone = 6, it is 'MDT' (Mountain Daylight Time?) in database

        LSR with tz=0 or tz=6, followed by the zone (if any) according to NCEI database:
        ...
        if RyanSobashSanityCheck:
            ...
            if rpts.empty and sql_df.empty:
                ...
                logging.warning(
                    f"SPC has {len(rpts)} {event_type} reports, "
                    f"but Ryan's SQL database has {len(sql_df)}.")
                ...
                logging.info(
                    f"mtime of SPC reports file:    {datetime.datetime.fromtimestamp(epoch1).strftime('%c')}")
                logging.info(
                    f"mtime of Ryan's SQL database: {datetime.datetime.fromtimestamp(epoch2).strftime('%c')}")
            # Times don't all match. ignore index
            elif not all(sql_df["datetime"].to_numpy() == rpts["time"].to_numpy()):
                logging.info(
                    "Ryan's database has same # of reports but times aren't equal. "
                    "Can't easily compare the lat and lons")
                print(f"SPC\n{rpts}")
                print(f"Ryan's\n{sql_df}")
            else:
...
def plotgridded(
    ...
    if storm_reports.empty:
        logging.debug(
            "spc.plotgridded(): storm reports DataFrame is empty."
            " data_gridded will be all zeros.")
...
def fill_NaNs_with_neighbors2d(array2d: xarray.DataArray):
    """ 
...
def histogram2d_weighted(bearing, dist_from_center, azbins, rbins, data):
    ...
    da = xarray.DataArray(data=weightedh2 / n, coords=coords,
                          dims=coords.keys(), attrs=data.attrs, name=data.name)
    # Fill NaNs with neighbors
...
def polarkde(
    ...
    n3 = geo.inverse((originlon.m, originlat.m), np.column_stack(
        (storm_reports["slon"], storm_reports["slat"])))
    n3 = np.asarray(n3)  # convert cartopy MemoryView to ndarray
    storm_reports["range"] = n3[:, 0] / \
        1000.  # geo.inverse returns meters; convert to km
...
def polarplot(
    ...
    for event_type, xrpts in storm_reports.groupby("event_type"):
        ...
        # Feed axes.scatter magnitudes, not quantities. Quantities are clever
        # but axes.scatter uses their units as axes labels. Not good for polar plot.
    ...
    if add_legend:
        storm_rpt_legend_kw = dict(fontsize=4.8, bbox_to_anchor=(
            0.8, -0.01), loc='upper left', borderaxespad=0, frameon=False,
...
def plot(storm_reports, ax, scale=1, drawrange=0,
         alpha=0.5, colorbyfreq=False):
    ...
    for event_type, xrpts in storm_reports.groupby("event_type"):
        ...
        if drawrange > 0:
            ...
            # With lons and lats, specifying more than one dimension allows
            # individual points to be drawn.
            # Otherwise a grid of circles will be drawn.
            # It warns about using PlateCarree to approximate Geodetic. It still warps
            # the circles appropriately, so I think this is okay.
            within_range = ax.tissot(rad_km=drawrange.to("km").magnitude,
                                     lons=lons[np.newaxis],
                                     lats=lats[np.newaxis],
                                     facecolor=kwdict[event_type]["c"],
                                     alpha=0.4,
                                     label=str(drawrange) + " range")
...
def to_MET(df: pd.DataFrame, gribcode=None):
    ...
    # as in
    # https://github.com/dtcenter/METplus/blob/main_v3.1/parm/use_cases/model_applications/convection_allowing_models/read_ascii_storm.py
    ...
    if gribcode:
        ...
        df.loc[df['event_type'].str.lower() == 'torn', "Grib_Code"] = 197
