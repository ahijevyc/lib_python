...
GLMDIR = os.getenv("GLMDIR", "/glade/scratch/"+os.getenv("USER","ahijevyc")+"/")
...
def flashOK(ds, debug=False):
    ...
    event_parent_group_id_counter = collections.Counter(ds.event_parent_group_id.values)
...
def read(GLMfiles, qc=0, debug=False):
    ...
    if concat_all:
        ...
        flash_time_offsets = np.empty(0,dtype=ds.flash_frame_time_offset_of_first_event.dtype)
    ...
    ds = xarray.open_dataset(GLMfiles[0])
    das, coords = get_das(ds) # keep data variables and coordinates separate. Can QC data variables but Can't QC coordinates.

    # Combine GLM datasets
    ds_dict = dict() # Dictionary of empty lists to extend with numpy arrays. Keys are all data variables and coordinates.
    ...
    for ftype in das.keys():
        for da in das[ftype]:
            data_vars[da] = xarray.DataArray(data=ds_dict[da], dims=ftype_dim[ftype], attrs=ds[da].attrs)
    ...
    if qc >= 1:
        ...
        for ftype in ["group", "flash"]:
            ...
            ds[das[ftype]] = ds[das[ftype]].where(ds[ftype+"_quality_flag"] == 0)
            ds = ds.dropna(dim=ftype_dim[ftype])
