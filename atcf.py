...
class Atcf(pd.DataFrame):
    ...
    def write(self, ofile, fullcircle=False, append=False):
        ...
        sanity_check = self.sort_values(
            ["basin", "cy", "initial_time", "fhr", "rad"])
        assert self.equals(
            sanity_check), "wind radii thresholds not in ascending order"

        models = self["model"].unique()

        if fullcircle:
            # deal with fullcircle.
            self = self.groupby(["basin", "cy", "initial_time", "fhr", "rad"]).apply(
                fullcircle_windradii)
        ...
        for index, row in self.iterrows():
            ...
            # avoid turning 'nan' into 3 spaces below
            line += "  , " if np.isnan(
                row.technum) else "{:02.0f}, ".format(row.technum)
            ...
            # supposedly 1 character, but always 3 in official b-decks
            line += "{:>3s}, ".format(row.subregion)
            ...
            for userdefine in ["userdefine"+n for n in ['1', '2', '3', '4', '5']]:
                ...
                if userdefine in row and row[userdefine].strip():
                    userdata = userdefine.replace("define", "data")
                    # Described as 1-20 chars in atcf doc.
                    line += "{:s}, ".format(row[userdefine])
                    # described as 1-100 chars in atcf doc
                    line += "{:s}, ".format(row[userdata])
        ...
        # possible columns with np.nan all happen to be 3 character columns
...
def decorate_ax(ax, bscale='50m'):
    ...
    ax.add_feature(cartopy.feature.LAKES,
                   facecolor=idl_water_color, alpha=alpha)
    ax.add_feature(cartopy.feature.STATES.with_scale(
        bscale), lw=0.25, edgecolor='gray')
...
def iswind_radii_method(s):
    ...
    if not float(s[:-10]):
        raise argparse.ArgumentTypeError(
            "First part of string must be a number")
    return s
...
def archive_path(atcfname):  # used by NARR_composite.py
    ...
    cy = atcfname[3:5]
    year = atcfname[5:9]
    apath = "/glade/work/ahijevyc/atcf/archive/"+year + \
        "/"+atcfname+".dat"  # provide path to atcf archive
    return apath


def cyclone_phase_space_columns():
    names = []
    # Cyclone Phase Space "Parameter B" for thermal asymmetry. (Values are *10)
    names.append('cpsB')
    # Cyclone Phase Space lower level (600-900 mb) thermal wind parameter, for diagnosing low-level warm core. (Values are *10)
    names.append('cpsll')
    # Cyclone Phase Space upper level (300-600 mb) thermal wind parameter, for diagnosing upper-level warm core. (Values are *10)
    names.append('cpsul')
    return names
...
def V500c(Vmax, latitude_degrees):
    ...
    shape_factor_x = 0.1147 + 0.0055 * Vmax_knots - \
        0.001 * (latitude_degrees - 25.)
    Rmax = 66.785 - 0.09102 * Vmax_knots + 1.0619 * (latitude_degrees - 25.)
    climatological_tangential_wind_500_km_from_the_center = Vmax_knots * \
        (Rmax/500.)**shape_factor_x
...
def Knaff_Zehr_Pmin(Vsrm1, storm_size_S, latitude_degrees, environmental_pressure):
    ...
    Pc = 23.286*hPa - 0.483*units.parse_expression("hPa/knot") * Vsrm1 - (Vsrm1/(24.254*units.parse_expression(
        "knot * hPa**-0.5")))**2. - 12.587*hPa*storm_size_S - 0.483*units.parse_expression("hPa/degrees_N")*latitude_degrees + environmental_pressure
    return Pc
...
def read(ifile, fullcircle=False):
    ...
    if fullcircle:
        ...
        df = df.groupby(["basin", "cy", "initial_time", "fhr",
                        "rad"]).apply(fullcircle_windradii)
...
def derived_winds(u10, v10, mslp, lonCell, latCell, row, vmax_search_radius=250.*units.km, mslp_search_radius=100.*units.km,
    ...
    derived_winds_dict = {  # these values were xarrays, but I applied .data() to them already.
        ...
        # used for Zehr and Knaff Pmin derivation.
        "storm_size_S": storm_size_S.to_base_units(),
