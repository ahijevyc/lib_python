...
def raw_vitals(row, diagdir, lonCell, latCell, wind_radii_method=None):
    ...
    # Don't choke on 'dBZ' not defined in unit registry
    ds = ds[['u10', 'v10', 'mslp']].metpy.quantify()
    ds = ds.isel(Time=0)

    # Extract vmax, RMW, minp, and radii of wind thresholds
    derived_winds_dict = atcf.derived_winds(
        ds.u10, ds.v10, ds.mslp, lonCell, latCell, row, wind_radii_method=wind_radii_method)
...
def origmesh(df: pd.DataFrame, initfile, diagdir, wind_radii_method="max"):
    """
    ...
    df = df.groupby('fhr').apply(raw_vitals, diagdir, lonCell,
                                 latCell, wind_radii_method=wind_radii_method)
...
for ztop in ['6', '1']:
    fieldinfo['shr0'+ztop+'mag']['fname'] = ['uzonal_'+ztop+'km',
                                             'umeridional_'+ztop+'km', 'uzonal_surface', 'umeridional_surface']
    fieldinfo['shr0'+ztop+'mag']['filename'] = 'diag'
fieldinfo['zlfc']['fname'] = ['lfc']
fieldinfo['zlcl']['fname'] = ['lcl']
# only zlcl filename needed to be changed from upp, not zlfc
fieldinfo['zlcl']['filename'] = 'diag'
for plev in ['200', '250', '300', '500', '700', '850', '925']:
    fieldinfo['hgt'+plev]['fname'] = ['height_'+plev+'hPa']
    fieldinfo['speed'+plev]['fname'] = ['uzonal_' +
                                        plev+'hPa', 'umeridional_'+plev+'hPa']
...
for plev in ['500', '700', '850']:
    ...
    fieldinfo['vort'+plev] = {'levels': [0, 9, 12, 15, 18, 21, 24, 27, 30, 33],
                              'cmap': readNCLcm('prcp_1'), 'fname': ['vorticity_'+plev+'hPa'], 'filename': 'diag'}
fieldinfo['vortpv'] = {'levels': [0, 9, 12, 15, 18, 21, 24, 27, 30, 33], 'cmap': readNCLcm(
    'prcp_1'), 'fname': ['vort_pv'],               'filename': 'diag'}
...
fieldinfo['stp']['fname'] = ['sbcape', 'lcl', 'srh_0_1km', 'uzonal_6km',
                             'umeridional_6km', 'uzonal_surface', 'umeridional_surface']
...
fieldinfo['shr06'] = {'fname': ['uzonal_6km', 'umeridional_6km',
                                'uzonal_surface', 'umeridional_surface'], 'filename': 'diag', 'skip': 50}
fieldinfo['shr01'] = {'fname': ['uzonal_1km', 'umeridional_1km',
                                'uzonal_surface', 'umeridional_surface'], 'filename': 'diag', 'skip': 50}

# Enter wind barb info for list of pressure levels
for plev in ['200', '250', '300', '500', '700', '850', '925']:
    fieldinfo['wind'+plev] = {'fname': ['uzonal_'+plev+'hPa',
                                        'umeridional_'+plev+'hPa'], 'filename': 'diag', 'skip': 50}
...
def makeEnsembleListMPAS(wrfinit, timerange, ENS_SIZE, g193=False):
    ...
    for hr in range(shr, ehr+1):
        wrfvalidstr = (wrfinit + datetime.timedelta(hours=hr)
                       ).strftime('%Y-%m-%d_%H.%M.%S')
        yyyymmddhh = wrfinit.strftime('%Y%m%d%H')
        for mem in range(1, ENS_SIZE+1):
            diag = '/glade/p/nsc/nmmm0046/schwartz/MPAS_ens_15-3km_mesh/POST/%s/ens_%d/diag.%s.nc' % (
                yyyymmddhh, mem, wrfvalidstr)
            if g193:
                diag = '/glade/p/nsc/nmmm0046/schwartz/MPAS_ens_15-3km_mesh/POST/%s/ens_%d/diag_latlon_g193.%s.nc' % (
                    yyyymmddhh, mem, wrfvalidstr)
            logging.debug(diag)
            if os.path.exists(diag):
                file_list['diag'].append(diag)
            else:
